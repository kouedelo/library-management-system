...
public class LibraryAssistantUtil {
    ...
    public static final String ICON_IMAGE_LOC = "/resource/icon.png";
    public static final String MAIL_CONTENT_LOC = "/resource/mail_content.html";
    private static final SimpleDateFormat DATE_TIME_FORMAT = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss a");
    ...
    public static Object loadWindowSettings(URL loc, String title, Stage parentStage, LibraryStaff staff) {
        SettingsController controller = null;
        try {
            FXMLLoader loader = new FXMLLoader(loc);
            Parent parent = loader.load();
            controller = loader.getController();
            controller.setCurrentStaff(staff);
            Stage stage = null;
            if (parentStage != null) {
                stage = parentStage;
            } else {
    ...
    public static Float getFineAmount(int totalDays, int maxDays) {
        Preferences pref = Preferences.getPreferences();
        Integer fineDays = totalDays - maxDays;
        Float fine = 0f;
        if (fineDays > 0) {
            fine = fineDays * 2.0f;
        }
